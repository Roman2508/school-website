    ...
    <div className="absolute top-full left-0 min-w-[220px] bg-white rounded-2xl shadow-card-hover border border-[hsl(80_15%_88%)] py-2 z-50">
      {items.map((item, i) => (
        ...
        >
          <Link
            href={item.link}
            className="flex items-center justify-between px-4 py-2.5 text-sm text-[hsl(0_0%_21%)] hover:text-[hsl(84_55%_45%)] hover:bg-[hsl(80_30%_93%)] transition-colors"
          >
            <span>{item.text}</span>
            ...
            <div className="absolute top-0 left-full min-w-[220px] bg-white rounded-2xl shadow-card-hover border border-[hsl(80_15%_88%)] py-2 z-50">
              {item.submenu.map((leaf) => (
                <Link
                  key={leaf.id}
                  href={leaf.link}
                  className="block px-4 py-2.5 text-sm text-[hsl(0_0%_21%)] hover:text-[hsl(84_55%_45%)] hover:bg-[hsl(80_30%_93%)] transition-colors"
                >
                  {leaf.text}
                </Link>
              ))}
...
function NavItem({ item }: { item: NavItemLevel1 }) {
  ...
  return (
    ...
    >
      <Link
        href={item.link}
        className="flex items-center gap-1 px-4 py-2 text-sm font-medium text-[hsl(0_0%_21%)] hover:text-[hsl(84_55%_45%)] hover:bg-[hsl(80_30%_93%)] rounded-xl transition-colors"
      >
        {item.title}
        {hasChildren && (
          <ChevronDown
    ...
    <div className="border-b border-[hsl(80_15%_88%)] last:border-0">
      <div className="flex items-center justify-between">
        <Link
          href={item.link}
          onClick={onClose}
          className="flex-1 px-4 py-3 text-sm font-semibold text-[hsl(0_0%_21%)] hover:text-[hsl(84_55%_45%)] transition-colors"
        >
          {item.title}
        </Link>
        {hasChildren && (
            ...
            <div key={l2.id}>
              <div className="flex items-center justify-between">
                <Link
                  href={l2.link}
                  onClick={onClose}
                  className="flex-1 py-2 pl-3 text-sm text-[hsl(0_0%_21%)] hover:text-[hsl(84_55%_45%)] transition-colors"
                >
                  {l2.text}
                </Link>
