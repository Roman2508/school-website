...
export default function Header({ data }: { data: HeaderData }) {
  ...
  const { navScrollRef, showScrollLeft, showScrollRight, scrollNavBy } = useNavScroll()
  ...
  return (
    ...
    >
      <div ref={topRef}>
        {data.announcement_bar_text && (
          <div className="bg-stats-gradient text-center py-2 px-4">
            <p className="text-sm font-medium text-white">{data.announcement_bar_text}</p>
          </div>
        )}
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0">
            ...
            {(data.title || data.subtitle) && (
              <div className="flex flex-col gap-1">
                {data.title && (
                  <span className="font-heading font-black text-base leading-none block">{data.title}</span>
                )}

                {data.subtitle && (
                  <span className="text-sm text-muted-foreground inline-block leading-[1.2]">{data.subtitle}</span>
                )}
              </div>
            )}
          </Link>
          ...
          <div className="hidden lg:flex items-center flex-1 px-4 min-w-0">
            <nav className="relative flex-1 min-w-0" aria-label="Головне меню">
              ...
              <button
                ...
                className={`absolute left-0 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/90 border border-[hsl(80_15%_88%)] shadow-card transition-opacity cursor-pointer ${
                  showScrollLeft ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
                }`}
              >
              ...
              <button
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 h-9 w-9 rounded-full bg-white/90 border border-[hsl(80_15%_88%)] shadow-card transition-opacity cursor-pointer ${
                  showScrollRight ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
                }`}
              >
      ...
      <div
        ...
        style={{
          maxHeight: mobileOpen ? `calc(100dvh - ${mobileMenuMaxHeight}px)` : '0px',
        }}
      >
